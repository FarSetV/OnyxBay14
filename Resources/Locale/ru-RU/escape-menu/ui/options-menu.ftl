...
ui-options-ambience-volume = Фоновая музыка:
...
ui-options-restart-sounds = Звуки перезапуске
...
ui-options-function-focus-admin-chat-window = Писать в чат (Admin)
ui-options-function-focus-dead-chat-window = Писать в чат (Dead)
...
ui-options-function-open-ahelp = Открыть ахелп
