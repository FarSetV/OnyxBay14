...
public sealed class PostMapInitTest
{
    ...
    [Test]
    [TestCaseSource(nameof(GetGameMapNames))]
    public async Task GameMapsLoadableTest(string mapProtoId)
    {
        await using var pairTracker = await PoolManager.GetServerClient(new PoolSettings { NoClient = true });
        var server = pairTracker.Pair.Server;
        ...
        await server.WaitPost(() =>
        {
            ...
            try
            {
                ticker.LoadGameMap(gameMap, mapId, null);
            }
            catch (Exception ex)
            {
                throw new Exception($"Failed to load map {mapProtoId}", ex);
            }
            ...
            {
                throw new Exception($"Failed to delete map {mapProtoId}", ex);
            }
        });
        await server.WaitRunTicks(1);
        ...
        using (ExecutionContext.SuppressFlow())
        {
            task = Task.Run(static async () =>
            {
                ...
                await using var pairTracker =
                    await PoolManager.GetServerClient(new PoolSettings { Disconnected = true });
                var server = pairTracker.Pair.Server;
                var resourceManager = server.ResolveDependency<IResourceManager>();
                var protoManager = server.ResolveDependency<IPrototypeManager>();
