...
public sealed class ClothingSystem : SharedClothingSystem
{
    ...
    private void OnGetVisuals(EntityUid uid, ClothingComponent item, GetEquipmentVisualsEvent args)
    {
        ...
        if (layers == null && !item.ClothingVisuals.TryGetValue(args.Slot, out layers))
        {
            ...
            if (!TryGetDefaultVisuals(uid, item, args.Slot, inventory.SpeciesId, humanoid?.BodyType, out layers))
                return;
        }
    ...
    private bool TryGetDefaultVisuals(EntityUid uid, ClothingComponent clothing, string slot, string? speciesId,
        string? bodyTypeId, [NotNullWhen(true)] out List<PrototypeLayerData>? layers)
    {
        layers = null;
        ...
        // body type specific
        if (bodyTypeId != null && rsi.TryGetState($"{state}-{bodyTypeId}", out _))
        {
            state = $"{state}-{bodyTypeId}";
        }
        // species specific
